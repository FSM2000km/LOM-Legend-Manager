...
def main() -> None:
    ...
    lines = [
        ...
        "",
        "ED名はゲーム内IDまたは手動確定から付与します。結縁相手は伝説に実際に保存された成立済みStory keyから判定し、想い人IDからは推測しません。生存、唐門加入、金烏上人死亡だけを通常の追加候補に表示し、その他のルート候補は`ネタバレタグを追加`を押すまで隠します。自動判定は、伝説に実際に保存されたStory keyと完全一致した肯定条件だけを使います。",
        "",
    ]
